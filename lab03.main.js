 ...
 * @return {stringobject} (firstIpAddress, ipv6) - An object containing 
                        both an IPv4 and its mapped IPv6 address
 */
function getFirstIpAddress(cidrStr, callback) {
  ...
  if (!cidr.isValid()) {
    ...
    addresses = {ipv4:firstIpAddress, ipv6};
  }
...
function main() {
  ...
  for (let i = 0; i < sampleCidrsLen; i++) {
    ...
    getFirstIpAddress(sampleCidrs[i], (data, error) => {
      ...
      console.log(`  Response returned from GET request: ${text}`);
    });
  }
